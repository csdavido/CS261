...
{
    // FIXME: implement
    int currentCap = hashMapCapacity(map);
    HashMap* tempMap = hashMapNew(capacity);

    for (int i = 0; i < currentCap; i++) {
        HashLink* new = map->table[i];

        while (new) {
           hashMapPut(tempMap, new->key, new->value);
           new = new->next;
        }
    }
...
void hashMapPut(HashMap* map, const char* key, int value)
{
    ...
    if (hashMapContainsKey(map, key)) {
        int* v = hashMapGet(map, key);
        (*v) += value;
    } else {
        HashLink* newValue = hashLinkNew(key, value, NULL);

        assert(newValue != NULL);

        if (map->table[index] == NULL) {
           map->table[index] = newValue;
        } else {
           HashLink* current = map->table[index];
           while(current->next) {
              current = current->next;
...
float hashMapTableLoad(HashMap* map)
{
    ...
    s = (float) hashMapSize(map);
    c = (float) hashMapCapacity(map);
