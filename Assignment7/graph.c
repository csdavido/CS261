...
int dfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
    ...
    if (source == destination) {
  	...
  	dequePushBack(newStack, source);

  	while (!located && !dequeIsEmpty(newStack)) {
  		Vertex *currentV = dequeBack(newStack);
  		dequePopBack(newStack);

  		currentV->isVisited = 1;

  		if (currentV == destination) {
  			located = 1;
  			break;
  		}
  		for (int i = 0; i < currentV->numNeighbors; i++) {
  			if (!(currentV->neighbors[i]->isVisited)) {
  				dequePushBack(newStack, currentV->neighbors[i]);
  			}
  		}
  	}
  	dequeDelete(newStack);
    return located;
}
...
int bfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
    ...
    if (source == destination) {
  	...
  	while (!located && !dequeIsEmpty(newQueue)) {
  		Vertex *currentV = dequeBack(newQueue);
  		dequePopBack(newQueue);
  		...
  		for (int i = 0; i < currentV->numNeighbors; i++) {
  			if (!(currentV->neighbors[i]->isVisited)) {
  				dequePushBack(newQueue, currentV->neighbors[i]);
  			}
  		}
  	}
  	dequeDelete(newQueue);
    return located;
}
